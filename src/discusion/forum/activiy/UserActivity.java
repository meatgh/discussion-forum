...
public class UserActivity {
	...
	public void seeAllQuestions(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		if ( questions.size() == 0 ) { // checking if there are questions at all for us to print
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		// Write else if condition to check if the user is a moderator
			if (user.getUserRole() == UserRole.MODERATOR) { //checking if the user is moderator
				if (question.getUser().getUserRole() == UserRole.USER) { //checking for condition 2 mentioned in the above MODERATOR case
					questionService.deleteQuestion(question);
				} else if (question.getUser() == user) { //checking for the condition 3
					questionService.deleteQuestion(question);
				} else {
					System.out.println("You are not authorised to delete this reply"); //when none of the above conditions meet the criteria
				}
		}
